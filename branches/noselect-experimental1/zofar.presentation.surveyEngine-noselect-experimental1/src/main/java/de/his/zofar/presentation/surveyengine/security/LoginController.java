...
public class LoginController implements Serializable {
	...
	public void setParticipantAuthenticationManager(ProviderManager participantAuthenticationManager) {
	...
	private ConcurrentSessionManager getConcurrentSessionManager() {
		...
		ConcurrentSessionManager manager = (ConcurrentSessionManager) context.getApplication()
				.evaluateExpressionGet(context, "#{concurrentSessionManager}", ConcurrentSessionManager.class);
		return manager;
	}
	...
	public boolean isReLogin() {
		...
		if (httpRequest != null) {
			...
			if (cookies != null) {
				for (final Cookie cookie : cookies) {
//					LOGGER.info("[COOKIE2] Name : {}", cookie.getName());
//					LOGGER.info("[COOKIE2] Domain : {}", cookie.getDomain());
//					LOGGER.info("[COOKIE2] MaxAge : {}", cookie.getMaxAge());
//					LOGGER.info("[COOKIE2] Path : {}", cookie.getPath());
//					LOGGER.info("[COOKIE2] Value : {}", cookie.getValue());
					if (cookie.getName().equals("relocation_sid")) {
						if ((cookie.getValue() != null) && (!cookie.getValue().equals(""))) {
							oldId = cookie.getValue();
							count = count + 1;
...
//				LOGGER.info("[COOKIE2] NO COOKIES");
			}
			if (count == 1) {
				if ((oldId != null) && (!oldId.equals(""))) {
					return true;
...
//				LOGGER.info("found more than one cookie");
			}
		} else {
//			LOGGER.info("[COOKIE2] NO REQUEST");
		}
		return false;
	}
		...
		if (!token.equals("")) {
			...
			try {
				...
				if (result.isAuthenticated()) {
					final List<SessionInformation> knownSessions = sessionRegistry.getAllSessions(result.getPrincipal(),
							false);
					...
					if (timeoutIntervall > 0)
						((HttpSession) session).setMaxInactiveInterval(timeoutIntervall);
					sessionRegistry.registerNewSession(RequestContextHolder.currentRequestAttributes().getSessionId(),
							result.getPrincipal());
					redirectHandler.sendRedirect(httpRequest, httpResponse, "/index.html");
					getConcurrentSessionManager().set(token,
							RequestContextHolder.currentRequestAttributes().getSessionId(), relogin);
				}
			} catch (AuthenticationException e) {
				LOGGER.info("Authentication failed: {}", token);
				try {
					redirectHandler.sendRedirect(httpRequest, httpResponse,
							"/special/login.html?message=AuthentificationFailed&zofar_token=" + token);
