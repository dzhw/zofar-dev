...
public class SurveyEngineServiceImpl implements SurveyEngineService {
	...
	// private Map<String,ParticipantEntity> partCache;
	//
	// @Transactional
	// public Participant saveSurveyParticipant(final Participant
	// participant,final List<String> dirty) {
	// LOGGER.info("saveSurveyParticipant  {} {}",participant.getToken(),dirty);
	// if(partCache == null)partCache = new HashMap<String,ParticipantEntity>();
	//
	// /* mapping from given participant-model to participant-entity */
	// ParticipantEntity partEntity = null;
	// if(partCache.containsKey(participant.getToken()))partEntity =
	// partCache.get(participant.getToken());
	// if(partEntity == null){
	// partEntity = new ParticipantEntity();
	// partEntity.setId(participant.getId());
	// partEntity.setPassword(participant.getPassword());
	...
	// partEntity.setVersion(participant.getVersion());
	//
	// ParticipantEntity savedPartEntity =
	// this.participantDao.saveAndFlush(partEntity);
	// partCache.put(participant.getToken(), savedPartEntity);
	// partEntity = savedPartEntity;
	// }
	//
	// for (final Map.Entry<String, SurveyData> entry : participant
	// .getSurveyData().entrySet()) {
	// if (entry != null) {
	// if(!dirty.contains(entry.getValue().getVariableName()))continue;
	// final SurveyDataEntity entity = new SurveyDataEntity();
	// entity.setId(entry.getValue().getId());
	// entity.setValue(entry.getValue().getValue());
	// entity.setVariableName(entry.getValue().getVariableName());
	...
	// entity.setParticipant(partEntity);
	//
	// final SurveyDataEntity savedEntity =
	// this.surveyDataDao.saveAndFlush(entity);
	//
	// if(savedEntity != null){
	// final SurveyData reloadedentity = new SurveyData();
	// reloadedentity.setId(savedEntity.getId());
	...
	// }
	// }
	// final ParticipantEntity savedEntity =
	// this.participantDao.saveAndFlush(partEntity);
	//
	// /* mapping back of entiteis to model */
