...
abstract class AbstractXhtmlElementCreator {
	...
	protected static final String ITEM_CLASSES = "zo-odd zo-even";
	...
	protected void setIdentifier(final IdentificationalType source, final com.sun.java.jsf.composite.common.IdentificationalType target) throws XmlException {
		// target.setId(generateUid(source));
		target.setId(source.getUid().trim());
		enhanceExpressions(source);

	}
	
	private void enhanceExpressions(final IdentificationalType source) {
		if(PageManager.getInstance().getMojo().isOverrideRendering()){
			enhanceExpressions(source,"visible");
		}
//		enhanceExpressions(source,"condition");
//		enhanceExpressions(source,"variable");
//		enhanceExpressions(source,"value");
//		enhanceExpressions(source,"sortCondition");
	}

	private void enhanceExpressions(final IdentificationalType source,final String mode) {
		try {
//			logger.info("enhance "+source.getClass()+" ("+source.getClass().getMethods()+")");
			...
			if(value != null){
				...
				args[0] = "devProxy.analyze('"+generateUid(source)+"','"+org.apache.commons.lang3.StringEscapeUtils.escapeEcmaScript(value)+"','"+mode.toUpperCase()+"',true)";
//				logger.info("enhanced expression : "+args[0]);
	...
	// * @return
	// */
	// protected String createVariableReference(final
	// de.his.zofar.xml.questionnaire.IdentificationalType uid,final String
	// variableName) {
	// return createElExpression(uid,variableName);
	// }
	...
	// * @return
	// */
	// protected String createElExpression(final
	// de.his.zofar.xml.questionnaire.IdentificationalType uid,final String
	// expression) {
	// return String.format(EL_PATTERN, expression);
	// }
	...
	protected void addTextToComparsionUnit(final de.his.zofar.xml.questionnaire.TextType source, final com.sun.java.jsf.composite.composite.ComparisonUnitType targetUnit) throws XmlException {
		...
		// logger.info("sourceclass {} xhtmlText {}",sourceClass,xhtmlText);
		if (xhtmlText != null)
			createText(source, xhtmlText);
	}
	...
	protected SortType createSortContainer(final SortableContainerType source, final SortType target) throws XmlException {
		...
		// FIXME use setIdentifier() instead.
		// target.setId(generateUid(source) + UID_DELIMITER + "sort");
		target.setId("sort");
	...
	public void addToOptionSection(IdentificationalType source, SectionType targetSection, boolean root) throws XmlException {
