...
public class TableRenderer extends Renderer {
	...
	private void encodeChildrenHelper(final FacesContext context, final UIComponent component, final List<UIComponent> children, final int colCount) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
	...
	private void addBody(final FacesContext context, final UIDisplayTableBody component, final ResponseWriter writer, final int colCount) throws IOException {
		...
		for (final UIComponent child : component.getChildren()) {
			if ((UIDisplayTableRow.class).isAssignableFrom(child.getClass())) {
				this.addRow(context, (UIDisplayTableRow) child, writer, colCount);
			}
		}
	}
	...
	private void addRow(final FacesContext context, final UIDisplayTableRow component, final ResponseWriter writer, final int colCount) throws IOException {
		...
		writer.startElement("tr", component);
		writer.writeAttribute("id", component.getClientId(), null);
		writer.writeAttribute("class", "zo-display-row-" + this.rowCounter, null);
		int lft = 0;
		for (final UIComponent child : component.getChildren()) {
			if ((UIDisplayTableItem.class).isAssignableFrom(child.getClass())) {
				this.addItem(context, (UIDisplayTableItem) child, writer, colCount);
				lft = lft + 1;
			}
		}
		while (lft < colCount) {
