...
public class ZofarLikertResponseDomainRenderer extends ZofarSingleChoiceResponseDomainRenderer {

	public static final String RENDERER_TYPE = "org.zofar.LikertResponseDomain";

	private static final Logger LOGGER = LoggerFactory.getLogger(ZofarLikertResponseDomainRenderer.class);

	protected void encodeInput(final FacesContext context, final UIComponent component) throws IOException {
	...
	protected List<UIComponent> retrieveLabels(final FacesContext context, final UIComponent component, final UIComponent child) throws IOException {
		...
		if ((Section.class).isAssignableFrom(child.getClass())) {
			...
			for (final UIComponent sectionChild : ((Section) child).getChildren()) {
				...
				back.addAll(this.retrieveLabels(context, component, sectionChild));
			}
			...
			for (final UIComponent sortChild : ((UISort) child).getChildren()) {
				...
				back.addAll(this.retrieveLabels(context, component, sortChild));
			}
		} else {
