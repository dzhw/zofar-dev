...
import de.his.zofar.presentation.surveyengine.ui.components.question.composite.doublematrix.UIDoubleMatrixUnit;
 ...
 */
@Deprecated
//@FacesRenderer(componentFamily = UIDoubleMatrixResponseDomain.COMPONENT_FAMILY, rendererType = OldZofarDoubleMatrixResponseDomainRenderer.RENDERER_TYPE)
public class OldZofarDoubleMatrixResponseDomainRenderer extends ZofarAbstractDoubleMatrixRenderer {

	private int row;

//	public static final String RENDERER_TYPE = "org.zofar.DoubleMatrixResponseDomain";

	private static final Logger LOGGER = LoggerFactory.getLogger(OldZofarDoubleMatrixResponseDomainRenderer.class);
	...
	public OldZofarDoubleMatrixResponseDomainRenderer() {
		super();
	}
	...
	public synchronized void encodeBegin(final FacesContext context, final UIComponent component) throws IOException {
		...
		if (this.isShowValues(context, component)) {
			final UIDoubleMatrixItem item = this.findFirstItem(component.getChildren());
			if (item != null) {
				writer.startElement("tr", component);
	 ...
	 * @throws IOException
	 */
	private synchronized void renderValuesHeader(final FacesContext context, final UIComponent component, final UIDoubleMatrixItem item, final Side side) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
		...
		switch (side) {
		case LEFT:
			facetName = ZofarAbstractDoubleMatrixRenderer.LEFT_QUESTION;
			break;
		case RIGHT:
			facetName = ZofarAbstractDoubleMatrixRenderer.RIGHT_QUESTION;
			break;
		default:
		...
		if ((question != null) && UISingleChoiceMatrixItemResponseDomain.class.isAssignableFrom(question.getClass())) {
	 ...
	 * @throws IOException
	 */
	private synchronized void renderHeader(final FacesContext context, final UIComponent component, final Side side) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
		...
		if (headerFacet != null) {
			...
			if (UIText.class.isAssignableFrom(headerFacet.getClass())) {
				// header facet doesn't have children therefore the facet itself
				// is the text.
				writer.startElement("th", component);
	...
	private synchronized void renderMissingHeader(final FacesContext context, final UIComponent component, final Side side) throws IOException {
	 ...
	 * @param side
	 */
	private synchronized void renderMissingHeader(final FacesContext context, final UIComponent component, final Side side, final Boolean isEmpty) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
		...
		if (missingHeaderFacet != null) {
			if (UIText.class.isAssignableFrom(missingHeaderFacet.getClass())) {
				...
				for (final UIComponent header : missingHeaderFacet.getChildren()) {
					writer.startElement("th", component);
					writer.writeAttribute("class", "zofar-missing", null);
					writer.writeAttribute("style", this.getHeaderMinWidth(context, component), null);
					if (!isEmpty) {
	 ...
	 * @param context
	 */
	private synchronized void renderScaleHeader(final FacesContext context, final UIComponent component, final Side side) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
		...
		if (scaleHeaderFacet != null) {
			...
			final int colspan = this.isDifferential(context, component) ? this.getNumberOfResponseOptions(context, component) + 2 : this.getNumberOfResponseOptions(context, component);
		...
		// render additional empty header for missing categories
	 ...
	 * @param context
	 */
	private synchronized void renderEmptyHeader(final FacesContext context, final UIComponent component) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
	...
	public synchronized void encodeChildren(final FacesContext context, final UIComponent component) throws IOException {
		// this.rowClasses = this.getRowClassesArray();

		this.renderItems(context, component, component.getChildren());
	}

	private synchronized void renderItems(final FacesContext context, final UIComponent component, final List<UIComponent> items) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();

//		final String[] rowClasses = this.itemClassesToArray((String) component.getAttributes().get("itemClasses"));
		final String[] rowClasses = new String[0];
		...
		for (final UIComponent child : items) {
			if (UIDoubleMatrixItem.class.isAssignableFrom(child.getClass())) {
				...
				if (hasRowClasses) {
					...
					classes += rowClasses[this.row++ % rowClasses.length] + " ";
				}
				if (!classes.equals("")) {
	...
	private synchronized void renderUnit(final FacesContext context, final UIComponent component, final UIDoubleMatrixUnit unit) throws IOException {
		...
		if (unitHeader != null) {
			...
			// the unit header must be centered in the table. additional column
			// span (1) is for the question column.
			final int colspan = this.isDifferential(context, component) ? ((this.getNumberOfResponseOptions(context, component) + 2) * 2) + 1 : (this.getNumberOfResponseOptions(context, component) * 2) + 1;
	...
	public synchronized Boolean isDifferential(final FacesContext context, final UIComponent component) {
		...
		if (component.getAttributes().get("isDifferential") != null) {
			isDifferential = (Boolean) component.getAttributes().get("isDifferential");
		}
	...
	private synchronized String getHeaderMinWidth(final FacesContext context, final UIComponent component) {
		// TODO le: replace the magic number 700
		return "width: " + (700 / (((this.getNumberOfResponseOptions(context, component) * 2) + this.getNumberOfMissings(context, component)))) + "px;";
	}
