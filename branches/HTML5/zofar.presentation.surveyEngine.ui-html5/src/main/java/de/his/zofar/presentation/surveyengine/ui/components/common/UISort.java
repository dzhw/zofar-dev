...
public class UISort extends UINamingContainer {
	...
	public void encodeChildren(final FacesContext context) throws IOException {
		for (final UIComponent child : this.sortChildren()) {
	...
	public List<UIComponent> sortChildren() {
		final List<UIComponent> childrenToSort = new ArrayList<UIComponent>(this.getChildren());
		final List<UIComponent> fromBean = this.getBean().sort(childrenToSort, this.getClientId(this.getFacesContext()), this.getMode());
		return fromBean;
	}
	...
	public ISortBean getBean() {
		return (ISortBean) this.getStateHelper().eval(PropertyKeys.bean, new SortBean());
	}
	...
	public boolean isSorted() {
		return Boolean.valueOf(this.getStateHelper().eval(PropertyKeys.sorted, Boolean.TRUE).toString());
	}
	...
	private class SortBean implements ISortBean {
		 ...
		 */
		@Override
		public List<UIComponent> sort(final List<UIComponent> toSort, final String parentId, final String mode) {
