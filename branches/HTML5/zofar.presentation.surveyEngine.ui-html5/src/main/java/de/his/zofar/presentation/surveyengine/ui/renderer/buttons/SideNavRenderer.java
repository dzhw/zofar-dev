...
public class SideNavRenderer extends Renderer {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(SideNavRenderer.class);
	...
	public void encodeChildren(final FacesContext context, final UIComponent component) throws IOException {
		...
		final Stack<List<Object>> stack = new Stack<List<Object>>();
		stack.add(tree);
		...
		while (it.hasNext()) {
			...
			if (Jumper.class.isAssignableFrom(child.getClass())) {
				...
				if (level == (stack.size() - 1)) {
					...
					// LOGGER.info("level : {} stack {}",level,(stack.size() -
					// 1));
		...
		this.encodeChildrenHelper(context, component, tree);
	}

	private void encodeChildrenHelper(final FacesContext context, final UIComponent component, final List<Object> children) throws IOException {
		if (!component.isRendered()) {
			return;
		}
		...
		while (it.hasNext()) {
			...
			if (Jumper.class.isAssignableFrom(child.getClass())) {
				...
				writer.startElement("li", component);
				this.encodeChildrenHelper(context, component, (List) child);
				writer.endElement("li");
			}
		}
