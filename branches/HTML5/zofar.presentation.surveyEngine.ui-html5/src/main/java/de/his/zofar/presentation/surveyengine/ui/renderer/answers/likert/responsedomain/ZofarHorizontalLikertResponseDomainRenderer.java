...
public class ZofarHorizontalLikertResponseDomainRenderer extends ZofarLikertResponseDomainRenderer {

	public static final String RENDERER_TYPE = "org.zofar.HorizontalLikertResponseDomain";

	private static final Logger LOGGER = LoggerFactory.getLogger(ZofarHorizontalLikertResponseDomainRenderer.class);
	private int row;
...
//		this.startTable(context.getResponseWriter(), component, additonalClasses);
	...
	public synchronized void encodeChildren(final FacesContext context, final UIComponent component) throws IOException {
		...
		// retrieve labels
		final List<UIComponent> labels = new ArrayList<UIComponent>();

		for (final UIComponent child : component.getChildren()) {
			labels.addAll(this.retrieveLabels(context, component, child));
		}
		...
		for (final UIComponent child : component.getChildren()) {
			...
			if (isMissing) {
				...
				this.encodeChildrenHelper(context, component, child);
			}
		}
	...
	public synchronized void encodeChildrenHelper(final FacesContext context, final UIComponent component, final UIComponent child) throws IOException {

		if (!child.isRendered()) {
			return;
		}
//		final String[] rowClasses = this.itemClassesToArray((String) component.getAttributes().get("itemClasses"));
		final String[] rowClasses = null;
//		final boolean hasRowClasses = rowClasses.length > 0;
		final boolean hasRowClasses = false;
		...
		if ((Section.class).isAssignableFrom(child.getClass())) {
			...
			for (final UIComponent sectionChild : ((Section) child).getChildren()) {
				...
				this.encodeChildrenHelper(context, component, sectionChild);
			}
			...
			for (final UIComponent sortChild : ((UISort) child).sortChildren()) {
				...
				this.encodeChildrenHelper(context, component, sortChild);
			}
		} else {
			Boolean isMissing = (Boolean) child.getAttributes().get("missing");
			if (isMissing == null) {
			...
			if (hasRowClasses) {
				classes.append(rowClasses[this.row++ % rowClasses.length] + " ");
			}
			if (isMissing) {
				classes.append("zo-ao-missing zo-ao-missing-likert-horizontal ");
			}
			if (!classes.toString().trim().equals("")) {
				writer.writeAttribute("class", classes.toString().trim(), null);
			}
			writer.writeAttribute("valign", "bottom", null);
