 ...
 * 
 */
class XhtmlQuestionSingleChoiceCreator extends AbstractXhtmlQuestionCreator implements IXhtmlCreator {

	private static final Logger LOGGER = LoggerFactory.getLogger(XhtmlQuestionSingleChoiceCreator.class);
	...
	public void addToSection(final IdentificationalType source, final SectionType targetSection, final boolean root) throws XmlException {
		...
		final Boolean isBooleanQuestion = ((QuestionSingleChoiceType) source).getResponseDomain().getType().equals(QuestionSingleChoiceResponseDomainType.Type.BOOLEAN);
	...
	private void createQuestion(final IdentificationalType source, final BaseQuestionType target) throws XmlException {
		final QuestionSingleChoiceType sourceQuestion = (QuestionSingleChoiceType) source;

		final Boolean isBooleanQuestion = sourceQuestion.getResponseDomain().getType().equals(QuestionSingleChoiceResponseDomainType.Type.BOOLEAN);
		...
		if (isBooleanQuestion) {
			addBooleanResponseDomain(sourceQuestion.getResponseDomain(), (BooleanType) target);
		} else {
			addResponseDomain(sourceQuestion.getResponseDomain(), (SingleChoiceType) target);
		}
	}
	 ...
	 * @throws XmlException
	 */
	private void addResponseDomain(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceType targetSingleChoice) throws XmlException {
		final QuestionSingleChoiceResponseDomainType.Type.Enum type = source.getType();
		...
		case QuestionSingleChoiceResponseDomainType.Type.INT_RADIO:
			xhtmlResponseDomain = createRadioResponseDomain(source, targetSingleChoice);
			break;
		case QuestionSingleChoiceResponseDomainType.Type.INT_DROPDOWN:
			xhtmlResponseDomain = createDropDownResponseDomain(source, targetSingleChoice);
			break;
		case QuestionSingleChoiceResponseDomainType.Type.INT_LIKERT:
			xhtmlResponseDomain = createLikertResponseDomain(source, targetSingleChoice);
			break;
		default:
			throw new IllegalStateException();
		}

		xhtmlResponseDomain.setValue(createVariableReference(source.getVariable() + ".valueId"));
		xhtmlResponseDomain.setVar(createVariableReference(source.getVariable()));
		...
		// added else where
		if (!type.equals(QuestionSingleChoiceResponseDomainType.Type.DROPDOWN) || !source.getMissingSeparated()) {
	 ...
	 * @throws XmlException
	 */
	private SingleChoiceResponseDomainType createDropDownResponseDomain(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceType targetSingleChoice) throws XmlException {

		SingleChoiceResponseDomainType xhtmlResponseDomain;
		boolean missingFlag = source.getMissingSeparated();
		if (missingFlag) {
			xhtmlResponseDomain = createDropDownMissingsResponseDomain(source, targetSingleChoice, missingFlag, true);
		} else {
			boolean hasAttached = false;
			final QuestionSingleChoiceAnswerOptionType[] answers = source.getAnswerOptionArray();
			...
			if (hasAttached) {
				xhtmlResponseDomain = createDropDownMissingsResponseDomain(source, targetSingleChoice, missingFlag, hasAttached);
			} else {
				xhtmlResponseDomain = targetSingleChoice.addNewComboSingleChoiceResponseDomain();
			}
		}
	 ...
	 * @throws XmlException
	 */
	private SingleChoiceResponseDomainType createDropDownMissingsResponseDomain(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceType targetSingleChoice, final boolean separateMissings, final boolean separateAttached) throws XmlException {

		final SingleChoiceResponseDomainType xhtmlResponseDomain = targetSingleChoice.addNewDropDownMissingResponseDomain();

		// the actual drop down list
		final FacetType dropdownFacet = ((DropDownMissingResponseDomainType) xhtmlResponseDomain).addNewFacet();
		...
		final Map<String, String> dropdownOptions = retrieveAdditionalAnswerOptions(source, true, false);
		...
		final FacetType missingFacet = ((DropDownMissingResponseDomainType) xhtmlResponseDomain).addNewFacet();
		missingFacet.setName(FacetType.Name.MISSING);
		final SingleChoiceResponseDomainType missing = missingFacet.addNewRadioButtonSingleChoiceResponseDomain();

		addAnswerOptions(source, missing, false, separateMissings, separateAttached, QuestionSingleChoiceResponseDomainType.Type.RADIO);
		// if (source.isSetItemClasses()) {
		...
		final Map<String, String> missingOptions = retrieveAdditionalAnswerOptions(source, false, separateMissings);
	 ...
	 * @return
	 */
	private SingleChoiceResponseDomainType createLikertResponseDomain(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceType targetSingleChoice) {
		SingleChoiceResponseDomainType xhtmlResponseDomain;

		xhtmlResponseDomain = targetSingleChoice.addNewLikertResponseDomain();
		if (source.isSetShowMidScaleHeader()) {
			((LikertResponseDomainType) xhtmlResponseDomain).setShowMidScaleHeader(source.getShowMidScaleHeader());
		}
		if (source.getDirection().equals(QuestionSingleChoiceResponseDomainType.Direction.HORIZONTAL)) {
			((LikertResponseDomainType) xhtmlResponseDomain).setDirection("horizontal");
		}
	 ...
	 * @return
	 */
	private SingleChoiceResponseDomainType createRadioResponseDomain(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceType targetSingleChoice) {
		SingleChoiceResponseDomainType xhtmlResponseDomain;

		switch (source.getDirection().intValue()) {
		case QuestionSingleChoiceResponseDomainType.Direction.INT_HORIZONTAL:
			xhtmlResponseDomain = targetSingleChoice.addNewRadioButtonSingleChoiceResponseDomain();
			((RadioButtonSingleChoiceResponseDomainType) xhtmlResponseDomain).setDirection("horizontal");
			((RadioButtonSingleChoiceResponseDomainType) xhtmlResponseDomain).setLabelPosition(getLabelPosition(source.getLabelPosition()));
			break;
		case QuestionSingleChoiceResponseDomainType.Direction.INT_VERTICAL:
			xhtmlResponseDomain = targetSingleChoice.addNewRadioButtonSingleChoiceResponseDomain();
			((RadioButtonSingleChoiceResponseDomainType) xhtmlResponseDomain).setDirection("vertical");
			((RadioButtonSingleChoiceResponseDomainType) xhtmlResponseDomain).setLabelPosition(getLabelPosition(source.getLabelPosition()));
			break;
		default:
			throw new IllegalStateException();
	 ...
	 * @return
	 */
	private Enum getLabelPosition(final QuestionSingleChoiceResponseDomainType.LabelPosition.Enum labelPosition) {
		...
		default:
			throw new IllegalArgumentException("unknown label position " + labelPosition);
		}
	}
	 ...
	 * @throws XmlException
	 */
	private void addAnswerOptions(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceResponseDomainType target, final QuestionSingleChoiceResponseDomainType.Type.Enum type) throws XmlException {
	 ...
	 * @throws XmlException
	 */
	private Map<String, String> retrieveAdditionalAnswerOptions(final IdentificationalType source, final Boolean addNonMissings, final Boolean addMissings) throws XmlException {
		final Map<String, String> back = new LinkedHashMap<>();
		for (final XmlObject child : source.selectPath(SELECT_PATH_IN_ORDER)) {
			if (QuestionSingleChoiceUnitType.class.isAssignableFrom(child.getClass())) {
				back.putAll(retrieveAdditionalAnswerOptions((QuestionSingleChoiceUnitType) child, addNonMissings, addMissings));
			} else if (QuestionSingleChoiceAnswerOptionType.class.isAssignableFrom(child.getClass())) {

				final Boolean isMissing = ((QuestionSingleChoiceAnswerOptionType) child).getMissing();

				if ((addNonMissings && !isMissing) || (addMissings && isMissing)) {
	 ...
	 * @throws XmlException
	 */
	private void addAnswerOptions(final QuestionSingleChoiceResponseDomainType source, final SingleChoiceResponseDomainType target, final Boolean addNonMissings, final Boolean addMissings, final Boolean addAttached, final QuestionSingleChoiceResponseDomainType.Type.Enum type) throws XmlException {
		...
		for (final XmlObject child : source.selectPath(SELECT_PATH_IN_ORDER)) {
			if (QuestionSingleChoiceUnitType.class.isAssignableFrom(child.getClass())) {
				...
				if (!addUnitRecursively((QuestionSingleChoiceUnitType) child, unit, addNonMissings, addMissings, addAttached, type)) {
					unit.getDomNode().getParentNode().removeChild(unit.getDomNode());
				}
			} else if (QuestionSingleChoiceAnswerOptionType.class.isAssignableFrom(child.getClass())) {
				...
				if ((addNonMissings && !isMissing && !isAttached) || (addMissings && isMissing) || (addAttached && isAttached)) {
					if (sort == null) {
	 ...
	 * @throws XmlException
	 */
	private boolean addUnitRecursively(final QuestionSingleChoiceUnitType source, final SectionType target, final Boolean addNonMissings, final Boolean addMissings, final Boolean addAttached, final QuestionSingleChoiceResponseDomainType.Type.Enum type) throws XmlException {
		boolean back = false;
		...
		for (final XmlObject child : source.selectPath(SELECT_PATH_IN_ORDER)) {
			if (QuestionSingleChoiceUnitType.class.isAssignableFrom(child.getClass())) {
				...
				if ((addNonMissings && !isMissing && !isAttached) || (addMissings && isMissing) || (!addNonMissings && addAttached && isAttached)) {
					if (sort == null) {
						singleOption = target.addNewSingleOption();
					} else {
						singleOption = sort.addNewSingleOption();
	 ...
	 * @throws XmlException
	 */
	private void addAnswerOption(final QuestionSingleChoiceAnswerOptionType source, final SingleOptionType target) throws XmlException {
	 ...
	 * @throws XmlException
	 */
	private void addBooleanResponseDomain(final QuestionSingleChoiceResponseDomainType source, final BooleanType targetBoolean) throws XmlException {
		final String variableReference = createVariableReference(source.getVariable());

		final BooleanResponseDomainType xhtmlResponseDomain = targetBoolean.addNewBooleanResponseDomain();

		setIdentifier(source, xhtmlResponseDomain);

		for (final QuestionSingleChoiceAnswerOptionType answerOption : source.getAnswerOptionArray()) {
			final SelectBooleanOptionType xhtmlBooleanOption = xhtmlResponseDomain.addNewSelectBooleanOption();
