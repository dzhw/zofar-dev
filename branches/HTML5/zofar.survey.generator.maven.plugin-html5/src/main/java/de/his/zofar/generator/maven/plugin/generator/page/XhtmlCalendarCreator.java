...
public class XhtmlCalendarCreator extends AbstractXhtmlElementCreator implements IXhtmlCreator {
	...
	// LoggerFactory.getLogger(XhtmlCalendarCreator.class);
	...
	private void createElement(IdentificationalType source, com.sun.java.jsf.composite.composite.CalendarType target)
		...
		target.setBehaviour(com.sun.java.jsf.composite.composite.CalendarType.Behaviour.Enum
				.forString(calendar.getBehaviour().toString()));

		//// final CalendarRowsType targetRows = target.addNewRows();
		//
		...
		//// final CalendarColumnsType targetColumns = target.addNewColumns();
		...
		// final CalendarLegendType targetLegend = target.addNewLegend();
...
//						legendItem.setColor(createElExpression(slotColor));
						legendItem.setColor(slotColor);
					}
					if (item.isSetPattern()) {
						final String slotPattern = item.getPattern();
//						legendItem.setPattern(createElExpression(slotPattern));
		...
		if (items != null) {
//			final CalendarItemsType targetItems = target.addNewItems();
