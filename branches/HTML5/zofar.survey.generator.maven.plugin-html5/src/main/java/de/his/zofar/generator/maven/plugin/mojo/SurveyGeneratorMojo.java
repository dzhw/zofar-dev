...
public class SurveyGeneratorMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		final PagesGeneratorMojo generator = new PagesGeneratorMojo(questionnaire, getLog(), webapp, resourceBundle, surveyResource, pretest, overrideRendering, noVisibleMap);
...
//	}
	...
	private QuestionnaireDocument appendQML(final QuestionnaireDocument base, final QuestionnaireDocument toAppend) {
		...
		if(appendPreloads != null) {
			...
			// indexing existing to prevent duplication 
			...
			for(final PreloadType preload : appendPreloads) {
				...
				if(!isNew) {
//					System.err.println("dublicate Preload found : "+preload.getName()+ " ("+preload.toString()+")");
					continue;
				}
		...
		if(appendVariables != null) {
			...
			for(final VariableType variable : appendVariables) {
				...
				if(!isNew) {
//					System.err.println("dublicate Variable found : "+variable.getName()+ " ("+variable.toString()+")");
					continue;
				}
