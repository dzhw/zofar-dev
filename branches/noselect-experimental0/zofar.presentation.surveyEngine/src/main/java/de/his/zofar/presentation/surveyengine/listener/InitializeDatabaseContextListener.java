...
public class InitializeDatabaseContextListener implements ServletContextListener {
	...
	// LOGGER.info("Preload {} for Participant {} already
	// exist.",variable,token);
	// }
	// else{
	// LOGGER.info("add Preload {} for Participant {}.",variable+" =
	// "+value,token);
	//
...
//					final String key = uid + "#" + variable;
//					if (!entities.containsKey(key)) {
//						if (answerOptionDao.findByVariableNameAndAnswerOptionUid(variable, uid) == null) {
//							entities.put(key, new AnswerOptionEntity(uid, variable));
//						}
//						else{
//							LOGGER.info("Allready there : {} , {}",variable, uid);
//						}
//					}
//					if (entities.containsKey(key))entities.get(key).addConditionAndKey(condition, bundlekey);
//				}
