...
public abstract class ZofarRadioSingleOptionRenderer extends Renderer{
	...
	protected boolean encodeOpenQuestion(final FacesContext context, final UIComponent component)
		...
		for (final UIComponent child : component.getChildren()) {
			...
			this.insertJavaScript(context,component,child.getClientId()+":"+child.getAttributes().get("inputId"));
			flag = true;
		}
		return flag;
	}
	
	private void insertJavaScript(final FacesContext context,final UIComponent component,final String oid) throws IOException {
		final ResponseWriter writer = context.getResponseWriter();
		writer.startElement("script", component);
		writer.writeAttribute("type", "text/javascript", null);
		final UIComponent responsedomain = JsfUtility.getInstance().getParentResponseDomain(component);
		final String script = "zofar_singleChoice_register_open('"+ responsedomain.getClientId() + "', '" + oid+"');\n";
		writer.write(script);
