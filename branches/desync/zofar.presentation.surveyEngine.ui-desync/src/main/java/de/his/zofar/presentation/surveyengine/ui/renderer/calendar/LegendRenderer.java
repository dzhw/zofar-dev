...
public class LegendRenderer extends Renderer {
	...
	public void encodeBegin(final FacesContext context, final UIComponent component) throws IOException {
		...
		if (!(UICalendarLegend.class).isAssignableFrom(component.getClass()))
			return;
		final boolean legendIconFlag = ((UICalendarLegend) component).getLegendIconAttribute();
	...
	public void encodeChildren(final FacesContext context, final UIComponent component) throws IOException {
		...
		final String layout = ((UICalendarLegend) component).getLayoutAttribute();
		final boolean indicatorFlag = ((UICalendarLegend) component).getIndicatorAttribute();
		final boolean legendIconFlag = ((UICalendarLegend) component).getLegendIconAttribute();
		...
		if (layout != null) {
			if (layout.equals("flowDirection")) {
				...
				for (final UIComponent item : component.getChildren()) {
					renderLegendItem(context, writer, item, component, layout, indicatorFlag, legendIconFlag,
							index + "");
					index = index + 1;
				}
			} else {
				...
				for (final Set<UIComponent> row : items) {
					...
					writer.writeAttribute("class", "zo-calendar-legend-page-row zo-calendar-legend-page-row-" + lft,
							null);
	...
	private Set<Set<UIComponent>> createColumnLayout(final int columnCount, final UIComponent component) {
		int count = 0;
	...
	private void renderLegendItem(final FacesContext context, final ResponseWriter writer, final UIComponent item,
			final UIComponent parent, final String layout, final boolean showIndicator, final boolean showLegendIcon,
		...
		if ((UISelectItem.class).isAssignableFrom(item.getClass())) {
			...
			if ((layout != null) && (!layout.equals("flowDirection"))) {
				...
				writer.writeAttribute("class", "zo-calendar-legend-cell zo-calendar-legend-cell-" + index, null);
			}
			...
			if ((layout != null) && (layout.equals("flowDirection"))) {
				...
				writer.writeAttribute("style", "margin-bottom:2px;margin-right:10px;position:relative;float:left;",
						null);
			} else {
			...
			if (showIndicator)
				jsTrigger += "toggleIndicator_" + parent.getParent().getId() + "('" + tmp.getItemValue() + "');";
			if (showLegendIcon) {
				final String uuid = UUID.randomUUID().toString().replaceAll("-", "");
				final String arrowId = "arrow" + uuid;
				final HtmlGraphicImage arrowRightImage = new HtmlGraphicImage();
				arrowRightImage.setValue(context.getApplication().evaluateExpressionGet(context,
						"#{resource['images/calendar/arrowRight.png']}", String.class));
				...
				String script = "registerPointer_" + item.getParent().getParent().getId() + "('"
						+ arrowRightImage.getId() + "');\n";
				writer.write(script);
				writer.endElement("script");
				jsTrigger += "setPointer_" + parent.getParent().getId() + "('" + arrowId + "');";
			}
			writer.endElement("td");
			...
			if ((pattern != null) && (!pattern.equals("")))
				colorCellStyle = colorCellStyle + "background-image:url('" + pattern + "');";
			writer.writeAttribute("style", colorCellStyle, null);
			writer.writeAttribute("onclick", jsTrigger + "setRadioWert_" + parent.getParent().getId() + "('"
					+ parent.getClientId(context) + "','" + tmp.getItemValue() + "');", null);
			writer.endElement("td");
			...
			if (!showLegendIcon) {
				...
				writer.writeAttribute("onclick",
						jsTrigger + "setNotPointer_" + parent.getParent().getId() + "('cell_" + uuid
								+ "');setRadioWert_" + parent.getParent().getId() + "('" + parent.getClientId(context)
								+ "','" + tmp.getItemValue() + "');",
						null);
				...
				writer.startElement("script", item);
				writer.writeAttribute("type", "text/javascript", null);
				String script = "registerNotPointer_" + item.getParent().getParent().getId() + "('cell_" + uuid
						+ "');\n";
				writer.write(script);
				writer.endElement("script");
			} else {
