 ...
 * 
 */
class XhtmlMatrixSingleChoiceCreator extends AbstractXhtmlMatrixCreator
		implements IXhtmlCreator {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(XhtmlMatrixSingleChoiceCreator.class);
	...
	public void createMatrix(final IdentificationalType source,
			final SingleChoiceMatrixType target) throws XmlException {
		...
		addResponseDomain(sourceMatrix.getResponseDomain(),
				target.addNewSingleChoiceMatrixResponseDomain());
	}
	...
	private void addResponseDomain(
			final MatrixQuestionSingleChoiceResponseDomainType source,
			final SingleChoiceMatrixResponseDomainType target)
			throws XmlException {
		...
		for (final XmlObject element : source.selectPath(SELECT_PATH_IN_ORDER)) {
			if (MatrixQuestionSingleChoiceItemType.class
			...
			} else if (MatrixQuestionSingleChoiceUnitType.class
					.isAssignableFrom(element.getClass())) {
				...
				addUnitRecursively(
						(MatrixQuestionSingleChoiceUnitType) element, unit);
			}
		}
	}

	private void addUnitRecursively(
			final MatrixQuestionSingleChoiceUnitType source,
			final SingleChoiceMatrixResponseDomainUnitType target)
			throws XmlException {
		...
		if (source.isSetHeader()) {
			...
			// LOGGER.info("add header {} {}",source.getHeader().getTitle(),header);
		...
		for (final XmlObject child : source.selectPath(SELECT_PATH_IN_ORDER)) {
			if (MatrixQuestionSingleChoiceUnitType.class.isAssignableFrom(child
					.getClass())) {
				SingleChoiceMatrixResponseDomainUnitType unit = null;
				if (sort == null) {
					unit = target.addNewSingleChoiceMatrixResponseDomainUnit();
				...
				addUnitRecursively((MatrixQuestionSingleChoiceUnitType) child,
						unit);
			} else if (MatrixQuestionSingleChoiceItemType.class
	 ...
	 * @throws XmlException
	 */
	private void addRowItem(final MatrixQuestionSingleChoiceItemType source,
			final SingleChoiceMatrixItemType target) throws XmlException {
		setIdentifier(source, target);
...
//			this.addAttachedOpenQuestion(source.getAttachedOpen(),target.addNewAttachedOpenQuestion());
		...
		// LOGGER.info("responseDomain type ({}) {} ",source.getUid(),source.getResponseDomain().getType());

		if (source.getResponseDomain().getType()
				.equals(QuestionSingleChoiceResponseDomainType.Type.RADIO)) {
			this.addRowItemResponseDomain(source.getResponseDomain(),
					target.addNewSingleChoiceMatrixItemResponseDomain());
		} else if (source.getResponseDomain().getType()
				.equals(QuestionSingleChoiceResponseDomainType.Type.DROPDOWN)) {
			if (source.getResponseDomain().getMissingSeparated()) {
				this.addRowItemResponseDomain(source.getResponseDomain(),
						target.addNewDropDownMissingResponseDomain());
			} else
				this.addRowItemResponseDomain(source.getResponseDomain(),
	...
	/*************************************************************************************/
	private boolean addUnitRecursivelyCombo(
			final QuestionSingleChoiceUnitType source, SectionType target,
		...
		for (final XmlObject element : source.selectPath(SELECT_PATH_IN_ORDER)) {
			...
			if (QuestionSingleChoiceUnitType.class.isAssignableFrom(element
					.getClass())) {
...
//				}
				...
				if (sort == null) {
					addRowOption(
							(QuestionSingleChoiceAnswerOptionType) element,
	 ...
	 * @throws XmlException
	 */
	private void addRowItemResponseDomain(
...
//					System.out.println("#93 variable: "+source.getVariable());
					addRowOption(
							(QuestionSingleChoiceAnswerOptionType) element,
							dropdown.addNewSingleOption(), true);
				} else {
					addRowOption(
							(QuestionSingleChoiceAnswerOptionType) element,
							sort.addNewSingleOption(), true);
		...
		// final SingleChoiceMatrixItemResponseDomainType missing =
		// missingFacet.addNewSingleChoiceMatrixItemResponseDomain();
		//
		// final Map<String,String> missingOptions = new HashMap<>();
		// for (final QuestionSingleChoiceAnswerOptionType option :
		// source.getAnswerOptionArray()) {
		...
		// MISSING_SUFFIX;
		// final VariableType missingVariable =
	...
	private void addRowItemResponseDomain(
			final QuestionSingleChoiceResponseDomainType source,
			final ComboSingleChoiceResponseDomainType target)
			throws XmlException {
...
//		System.out.println("#93 variable: "+source.getVariable());
	 ...
	 * @throws XmlException
	 */
	void addRowOption(final QuestionSingleChoiceAnswerOptionType source,
	 ...
	 */
	void addRowOption(final QuestionSingleChoiceAnswerOptionType source,
			final SingleOptionType target, final boolean addLabel)
			throws XmlException {
		setIdentifier(source, target);
