...
public class PagesGeneratorMojo {
	 ...
	 */
	public PagesGeneratorMojo(final QuestionnaireDocument questionnaire, final Log log, final File outputDirectory, final File messageBundleOutputDirectory, final File surveyOutputDirectory,final boolean pretest,final boolean overrideRendering) {
		super();
		this.log = log;
	...
	public final void execute() throws MojoExecutionException, MojoFailureException {
		...
		if (pages != null) {
			int currentPage = 0;
			// create XHTML file for all pages
			for (final PageType page : pages) {
				final String pageName = page.getUid();

				final ZofarWebPage webPage = new ZofarWebPage(pageName, this.calculateProgress(currentPage++, pages.length),overrideRendering);
	 ...
	 * @throws MojoFailureException
	 */
	public void savePage(final String projectPath, final PageType page, final String pageName, final ZofarWebPage webPage) throws MojoFailureException {
		try {
...
//				this.log.debug("create page " + pageName + " and saved.");
	...
	private void saveMessageBundle() throws IOException {
		...
		if (!this.msgBundleFile.exists()) {
			...
			// delete file, we want to create a whole new file
		...
		try {
			// saving message bundle properties
...
//		this.msgTranslation.put(bundleKey, cleanedText);
