...
public class W3CMarkupValidatorService {
	...
	public Map<TYPE, Object> validate(final Page page) {
		try {
			...
			Page resultPage = HTTPClient.getInstance().post("http://xxx/vnu/?out=json&parser=htm5", parameter, body);
			if (resultPage != null) {
	...
	// WebResponse response = page.getWebResponse();
	// String body = response.getContentAsString();
	// if(body.startsWith("<?xml version=\"1.0\" encoding=\"UTF-8\"?>"))body =
	// body.substring("<?xml version=\"1.0\" encoding=\"UTF-8\"?>".length());
	// final File debugFile = FileClient.getInstance().createTempFile("html",
	// ".html");
	// FileClient.getInstance().writeToFile(debugFile, body, false);
	//
	...
	// final JsonElement found = msg.get("extract");
	//
	// if(message.getAsString().startsWith("Malformed byte sequence:"))continue;
	//
	// JsonElement locationFirstLine = msg.get("firstLine");
	// JsonElement locationFirstColumn = msg.get("firstColumn");
	//
	...
	// final JsonElement locationLastColumn = msg.get("lastColumn");
	//
	// if(locationFirstLine == null)locationFirstLine = locationLastLine;
	// if(locationFirstColumn == null)locationFirstColumn = locationLastColumn;
	//
	// final W3CMessage tmpMsg = new W3CMessage(locationFirstColumn.getAsInt(),
	// locationFirstLine.getAsInt(),locationLastColumn.getAsInt(),
	// locationLastLine.getAsInt(), type.getAsString(),
	// message.getAsString(),found);
	// messageList.add(tmpMsg);
	// }
	// }
	//
